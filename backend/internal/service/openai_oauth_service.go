...
func ensureCookieKV(cookieHeader string, name string, value string) string {
	value = strings.TrimSpace(value)
	if value == "" {
		return strings.TrimSpace(cookieHeader)
	}
	if strings.Contains(strings.ToLower(cookieHeader), strings.ToLower(name)+"=") {
		return strings.TrimSpace(cookieHeader)
	}
	if strings.TrimSpace(cookieHeader) == "" {
		return name + "=" + value
	}
	return name + "=" + value + "; " + strings.TrimSpace(cookieHeader)
}
...
func (s *OpenAIOAuthService) RefreshAccountToken(ctx context.Context, account *Account) (*OpenAITokenInfo, error) {
	...
	if refreshToken != "" {
		...
		slog.Warn("openai_oauth_refresh_token_failed_falling_back_to_session", "account_id", account.ID, "error", err)
	}

	sessionToken := normalizeSoraSessionTokenInput(account.GetCredential("session_token"))
	if sessionToken != "" {
		return s.ExchangeChatGPTSessionToken(ctx, sessionToken, account.GetCredential("cookies"), proxyURL)
	}

	if refreshToken == "" {
		return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_NO_REFRESH_TOKEN", "no refresh token or session_token available")
	}
	return nil, infraerrors.New(http.StatusBadGateway, "OPENAI_OAUTH_REFRESH_FAILED", "oauth refresh failed and no session_token fallback available")
}

func (s *OpenAIOAuthService) ExchangeChatGPTSessionToken(ctx context.Context, sessionToken string, cookieHeader string, proxyURL string) (*OpenAITokenInfo, error) {
	sessionToken = normalizeSoraSessionTokenInput(sessionToken)
	if strings.TrimSpace(sessionToken) == "" {
		return nil, infraerrors.New(http.StatusBadRequest, "CHATGPT_SESSION_TOKEN_REQUIRED", "session_token is required")
	}

	cookieHeader = ensureCookieKV(strings.TrimSpace(cookieHeader), "__Secure-next-auth.session-token", sessionToken)

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, openAIChatGPTSessionAuthURL, nil)
	if err != nil {
		return nil, infraerrors.Newf(http.StatusInternalServerError, "CHATGPT_SESSION_REQUEST_BUILD_FAILED", "failed to build request: %v", err)
	}
	...
	if err != nil {
		return nil, infraerrors.Newf(http.StatusBadGateway, "CHATGPT_SESSION_CLIENT_FAILED", "create http client failed: %v", err)
	}
	resp, err := client.Do(req)
	if err != nil {
		return nil, infraerrors.Newf(http.StatusBadGateway, "CHATGPT_SESSION_REQUEST_FAILED", "request failed: %v", err)
	}
	defer func() { _ = resp.Body.Close() }()

	body, _ := io.ReadAll(io.LimitReader(resp.Body, 2<<20))
	if resp.StatusCode != http.StatusOK {
		return nil, infraerrors.Newf(http.StatusBadGateway, "CHATGPT_SESSION_EXCHANGE_FAILED", "status %d: %s", resp.StatusCode, strings.TrimSpace(string(body)))
	}
	...
	if err := json.Unmarshal(body, &sessionResp); err != nil {
		return nil, infraerrors.Newf(http.StatusBadGateway, "CHATGPT_SESSION_PARSE_FAILED", "failed to parse response: %v", err)
	}
	if strings.TrimSpace(sessionResp.AccessToken) == "" {
		return nil, infraerrors.New(http.StatusBadGateway, "CHATGPT_SESSION_ACCESS_TOKEN_MISSING", "session exchange response missing access token")
	}
	...
	return &OpenAITokenInfo{
		...
		ChatGPTUserID:    strings.TrimSpace(sessionResp.User.ID),

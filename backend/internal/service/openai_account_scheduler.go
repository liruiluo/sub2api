...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	selection, escapedSticky, err := s.selectBySessionHash(ctx, sessionReq)
	if err != nil {
		return nil, decision, err
	}
	if selection != nil && selection.Account != nil {
		...
		if !escapedSticky && !s.shouldLoadBalanceBusySessionSticky(req) {
			decision.Layer = openAIAccountScheduleLayerSessionSticky
			decision.StickySessionHit = true
			decision.SelectedAccountID = selection.Account.ID
			decision.SelectedAccountType = selection.Account.Type
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if !s.service.isOpenAIAccountInGroup(account, req.GroupID) {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, false, nil
	}
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrder(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		fresh := s.service.resolveFreshSchedulableOpenAIAccountForGroup(
			ctx, candidate.account, req.GroupID, req.RequestedModel, false, req.RequiredCapability)
...
func (s *defaultOpenAIAccountScheduler) shouldLoadBalanceBusySessionSticky(req OpenAIAccountScheduleRequest) bool {
	if req.RequiredTransport == OpenAIUpstreamTransportResponsesWebsocket ||
	...
	return wsCfg.Enabled && (wsCfg.ResponsesWebsockets || wsCfg.ResponsesWebsocketsV2)
}

...
func TestApplyCodexOAuthTransform_PreservesUnderscoreForOpenAICallIDs(t *testing.T) {
	reqBody := map[string]any{
		"model": "gpt-5.4",
		"input": []any{
			map[string]any{
				"type": "item_reference",
				"id":   "call_YYen1qxDejd2myJwcTCf7Nyp",
			},
			map[string]any{
	...
	require.True(t, ok)
	require.Equal(t, "fc_YYen1qxDejd2myJwcTCf7Nyp", ref["id"])

	output, ok := input[1].(map[string]any)
	require.True(t, ok)
	require.Equal(t, "fc_YYen1qxDejd2myJwcTCf7Nyp", output["call_id"])
}

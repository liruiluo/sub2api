...
func TestOpenAIGatewayService_APIKeyPassthrough_NormalizesBareCodexAliasForOfficialOpenAI(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"codex","stream":false,"input":[{"type":"text","text":"hi"}]}`)
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid"}},
		Body:       io.NopCloser(strings.NewReader(`{"output":[],"usage":{"input_tokens":1,"output_tokens":1,"input_tokens_details":{"cached_tokens":0}}}`)),
	...
	require.NotNil(t, result)
	require.NotNil(t, upstream.lastReq)
	require.JSONEq(t, `{"model":"gpt-5-codex","stream":false,"input":[{"type":"text","text":"hi"}]}`, string(upstream.lastBody))
	require.Equal(t, "https://api.openai.com/v1/responses", upstream.lastReq.URL.String())
}

func TestOpenAIGatewayService_APIKeyPassthrough_PreservesBareCodexAliasForCustomBaseURL(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := &Account{
		...
		Type:           AccountTypeAPIKey,
		Concurrency:    1,
		Credentials:    map[string]any{"api_key": "sk-api-key", "base_url": "https://openai-compatible.example.com/v1"},
		Extra:          map[string]any{"openai_passthrough": true},
		Status:         StatusActive,
		Schedulable:    true,
		RateMultiplier: f64p(1),
	...
	require.NotNil(t, upstream.lastReq)
	require.Equal(t, originalBody, upstream.lastBody)
	require.Equal(t, "https://openai-compatible.example.com/v1/responses", upstream.lastReq.URL.String())
}

func TestOpenAIGatewayService_APIKeyPassthrough_PreservesGPT5Alias(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"gpt-5","stream":false,"input":[{"type":"text","text":"hi"}]}`)
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid"}},
		Body:       io.NopCloser(strings.NewReader(`{"output":[],"usage":{"input_tokens":1,"output_tokens":1,"input_tokens_details":{"cached_tokens":0}}}`)),
...
func TestOpenAIGatewayService_APIKeyPassthrough_PreservesCodexMiniLatestAlias(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"codex-mini-latest","stream":false,"input":[{"type":"text","text":"hi"}]}`)
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid"}},
		Body:       io.NopCloser(strings.NewReader(`{"output":[],"usage":{"input_tokens":1,"output_tokens":1,"input_tokens_details":{"cached_tokens":0}}}`)),
...
func TestOpenAIGatewayService_APIKeyPassthrough_HTTP404RouteErrorTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := &Account{
		...
		Type:           AccountTypeAPIKey,
		Concurrency:    1,
		Credentials:    map[string]any{"api_key": "sk-api-key", "base_url": "https://broken.example"},
		Extra:          map[string]any{"openai_passthrough": true},
		Status:         StatusActive,
		Schedulable:    true,
		RateMultiplier: f64p(1),
	...
	require.Contains(t, string(failoverErr.ResponseBody), "page not found")
	require.False(t, c.Writer.Written(), "passthrough 404 路由错误应交给上层换号")
}

func TestOpenAIGatewayService_OAuthPassthrough_StreamReadErrorBeforeFirstTokenTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "X-Request-Id": []string{"rid-stream-eof"}},
	...
	_, err := svc.Forward(context.Background(), c, account, []byte(`{"model":"gpt-5.4","stream":true,"instructions":"x","input":[{"type":"text","text":"hi"}]}`))
	require.Error(t, err)

	var failoverErr *UpstreamFailoverError
	require.ErrorAs(t, err, &failoverErr)
	require.Equal(t, http.StatusBadGateway, failoverErr.StatusCode)
	require.False(t, c.Writer.Written(), "首 token 前上游断流应交给上层换号")
}

func TestOpenAIGatewayService_OAuthPassthrough_RequestErrorTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	repo := &openAIPassthroughFailoverRepo{}
	svc := &OpenAIGatewayService{
		cfg:          &config.Config{Gateway: config.GatewayConfig{ForceCodexCLI: false}, RateLimit: config.RateLimitConfig{OverloadCooldownMinutes: 10}},
		httpUpstream: upstream,
		accountRepo:  repo,
	}
	...
	result, err := svc.Forward(context.Background(), c, account, []byte(`{"model":"gpt-5.4","stream":false,"instructions":"x","input":[{"type":"text","text":"hi"}]}`))
	require.Nil(t, result)
	var failoverErr *UpstreamFailoverError
	require.ErrorAs(t, err, &failoverErr)
	require.Equal(t, http.StatusBadGateway, failoverErr.StatusCode)
...
func TestOpenAIGatewayService_OAuthPassthrough_PreCommitStreamFailureTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	result, err := svc.Forward(context.Background(), c, account, []byte(`{"model":"gpt-5.4","stream":true,"instructions":"x","input":[{"type":"text","text":"hi"}]}`))
	require.Nil(t, result)
	var failoverErr *UpstreamFailoverError
	require.ErrorAs(t, err, &failoverErr)
	require.Equal(t, http.StatusBadGateway, failoverErr.StatusCode)
	require.Contains(t, string(failoverErr.ResponseBody), "before first data event")
}
